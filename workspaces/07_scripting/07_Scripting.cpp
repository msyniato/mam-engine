...
#include <mam/scriptsys/script_system.hpp>
...
#pragma region ENGINE_SETUP
  ...
  mam::ScriptSystem scripts;
  scripts.init(world, input);

  mam::Signature scriptSignature;
  scriptSignature.set(world->getComponentType<mam::ScriptComponent>());

  auto* scriptSystem = world->registerSystem("script_system", scriptSignature);
  scriptSystem->onUpdate = mam::ScriptSystem::tick;
  ...
  for (int i = 0; i < kCubeCount; ++i) {
    ...
    if (i % 10 == 0) {
      world->addComponent<mam::ScriptComponent>(
        cube,
        mam::ScriptComponent{
          .file = "../assets/script/transform.lua"
        }
      );
  ...
  std::cout << "[07_scripting] " << kCubeCount << " cubes, "
    << scriptedCount << " with script\n";
