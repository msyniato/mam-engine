...
// dependencies: []
// priority: 5
// description: Disney BSDF evaluation functions for direct lighting (Vulkan)

// PARAMETERS
// (none — pure helper functions, no uniforms or varyings)
...
vec3 evalDisneySpecular(
    ...
    float HdotX, float HdotY, float LdotX, float LdotY, float VdotX, float VdotY
) {
    float luminance   = dot(baseColor, vec3(0.3, 0.6, 0.1));
    vec3  tint        = luminance > 0.0 ? baseColor / luminance : vec3(1.0);
    vec3  F0          = mix(specular * 0.08 * mix(vec3(1.0), tint, specularTint), baseColor, metallic);
...
vec3 evalDisney(
    ...
    vec3 diffuse     = evalDisneyDiffuse(baseColor, roughness, NdotL, NdotV, LdotH) * (1.0 - metallic);
    vec3 sheenC      = evalDisneySheen(baseColor, sheen, sheenTint, LdotH)           * (1.0 - metallic);
    vec3 spec        = evalDisneySpecular(
        baseColor, metallic, roughness, specular, specularTint, anisotropic,
        NdotL, NdotV, NdotH, LdotH,
        dot(H, X), dot(H, Y), dot(L, X), dot(L, Y), dot(V, X), dot(V, Y));
    float coat       = 0.25 * clearcoat * evalDisneyClearcoat(clearcoatGloss, NdotL, NdotV, NdotH, LdotH);

    return (diffuse + sheenC + spec) * NdotL + vec3(coat * NdotL);
}
